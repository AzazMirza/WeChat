<div >
    <div style="overscroll-behavior: none;">
        ...
        <div class="mt-20 mb-16 space-y-4"> {{-- Use `space-y-4` for vertical spacing between messages --}}
          @foreach ($message as $mess)
              @if ($mess['sender'] != auth()->user()->name) {{-- Receiver's message --}}
                  <div class="flex justify-start">
                      <div class="bg-gray-300 w-auto max-w-3/4 mx-4 my-2 p-2 rounded-lg">
                          <strong>{{ $mess['sender'] }}</strong>: {{ $mess['message'] }}
                      </div>
                  </div>
              @else {{-- Sender's message --}}
                  <div class="flex justify-end">
                      <div class="bg-green-300 w-auto max-w-3/4 mx-4 my-2 p-2 rounded-lg">
                          <strong>You</strong>: {{ $mess['message'] }}
                      </div>
                  </div>
              @endif
          @endforeach
